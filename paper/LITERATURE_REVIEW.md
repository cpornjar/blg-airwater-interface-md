# Literature Review — Two-Factor Gating of BLG Adsorption
...
- **Relevance:** Experimental evidence that AWI protein orientation → foam stability. Strong support for our orientation-gate hypothesis. Note: uses BSA not BLG, but in same context.
- **Citations:** 75
...
- **Relevance:** **Direct experimental + MD evidence** that BLG orientation at the AWI matters for adsorption. This is the closest prior work to our orientation gate. The BLG MD component of this paper (in electrolyte, not air-water slab) is the only prior MD study directly connecting BLG orientation to adsorption efficiency.
- **Cite where:** Introduction and sec:gate — "Experimental evidence that BLG orientation at the AWI modulates adsorption (Beierlein et al. 2015) is consistent with the orientation gate identified here."
...
- **Relevance:** Independent cryo-EM evidence that proteins have preferential orientations at AWI — directly supports the orientation gate concept. Note: preprint, treat with caution.
...
- **Relevance:** Supports the SASA/hydrophobic-exposure component of the gate: when BLG is forced into a state with more exposed hydrophobic surface (as nanoparticles), adsorption improves — consistent with the SASA ≥ 35 nm² activation threshold
- **Citations:** 9
...
| Protein orientation at AWI | Alamdari 2020, Engelhardt 2012, Beierlein 2015 | None used BLG from bulk at air–water |
...
| Molecular origin of kinetic barrier | Unknown | Two-factor gate: SASA ≥ 35 nm² AND θ ≤ 30° |
| Anti-correlation of gate factors | Not hypothesised | 3.3-fold below independent expectation (CI [0.11, 0.50]) |
...
1. **Direct validation:** No gate-open → commitment transition observed in unbiased MD (acknowledged; enhanced sampling needed)
...
| **Promising open direction** | Orientation-resolved MD of globular proteins at AWI; enhanced sampling for gate-open events; αLA/BLG comparison to test the calyx specificity hypothesis |
...
|----------|-------|--------------|-----|
| **HIGH** | Beierlein et al. 2015 (*JPCB*) | Intro §3 + sec:gate | Only prior work combining BLG MD + SFG showing orientation matters for adsorption |
| **HIGH** | Eberini et al. 2004 (*Proteins*) | sec:activation, Loop CD/EF paragraph | Only prior BLG MD study of calyx loop dynamics |
| **MEDIUM** | Engelhardt et al. 2012 (*Langmuir*) | sec:gate or Discussion | Experimental link: protein AWI orientation → foam stability |
| **MEDIUM** | Sun et al. 2024 (*Food Hydrocolloids*) | Discussion | SASA exposure → adsorption efficiency for BLG nanoparticles |
| **LOW** | Hingst et al. 2025 (*Eur Biophys J*) | Discussion | Very recent; mAb orientation at AWI disrupted by surfactants |

---

*Literature review completed May 27, 2026. Figure 3 regeneration still running (PID 5354). Next step: verify Fig 3 SASA values once regeneration completes, then consider adding Beierlein 2015 and Eberini 2004 citations to manuscript.*
