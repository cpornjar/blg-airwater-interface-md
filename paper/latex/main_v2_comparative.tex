% =====================================================================
% Paper 1 — COMFHA
% "An Accessible Calyx and an Open Chain: Contrasting Adsorption
%  Strategies of beta-Lactoglobulin and beta-Casein at the Air-Water
%  Interface"
% Target venue: Journal of Colloid and Interface Science (JCIS)
% Draft v4 (comparative) — June 12, 2026
%
% STATUS:
%   - BLG sections (Results 3.1-3.3, most of Discussion, BLG Methods)
%     are validated and carried over from v3 (May 28, "Contact without
%     Commitment", BLG-only). main.tex (v3) remains the paused BLG-only
%     submission candidate and is untouched.
%   - New in v4: calyx-residue-only SASA result (Sec. 3.3), beta-casein
%     "open-chain" comparison section (Sec. 3.4) and beta-casein Methods
%     are marked [DATA PENDING] -- CASEIN GROMACS setup begins next
%     session (system prep -> production -> analysis, see
%     docs/paper2_plan.md). The comparative Discussion and Conclusion
%     are gated entirely on those results.
% =====================================================================
...
% ---------------------------------------------------------------------
\title{\large An Accessible Calyx and an Open Chain: Contrasting
       Adsorption Strategies of \boldmath$\beta$-Lactoglobulin and
       $\beta$-Casein at the Air--Water Interface}
...
activation barrier. The calyx-lining residues themselves (the nine
residues forming the hydrophobic pocket) remain solvent-exposed
throughout, with SASA $\SI{3.81 \pm 0.46}{\nano\metre\squared}$ (CENTER,
PBC-corrected) and no collapse toward burial --- the calyx is an
\emph{accessible} feature of native BLG, not one that opens transiently
and recloses. \textbf{[DATA PENDING]} This BLG characterisation is one
half of a planned comparative study with $\beta$-casein, an
intrinsically disordered milk protein whose unstructured, amphipathic
chain is hypothesised to present a contrasting, ``open-chain''
adsorption strategy; $\beta$-casein simulations and analysis are in
progress (Methods) and the comparative synthesis is reported in a
forthcoming revision of this manuscript.
\end{abstract}

\vspace{6pt}

\noindent\textbf{Keywords:} $\beta$-lactoglobulin; $\beta$-casein;
air--water interface; molecular dynamics; protein adsorption; contact
ensemble; intrinsically disordered protein; foam stability; hydrophobic
exposure; CHARMM36m.
...
state\citep{Cornec1999,Mackie1999,Ulaganathan2017a,Foam4_2020,Perriman2007}. These methods, however,
...
BLG is not the only milk protein that matters at the air--water interface.
$\beta$-Casein, the other major component of the casein fraction, is an
intrinsically disordered protein (IDP): it has no stable tertiary fold,
and its sequence is dominated by a strongly amphipathic N-terminal region
(residues 1--25) thought to anchor the chain at hydrophobic/hydrophilic
boundaries\citep{Mackie1999}.~\footnote{Holt \& Sawyer's structural analysis
of the caseins (1993) is the standard reference for this sequence-level
description and will be added to \texttt{references.bib} and cited here
once read and annotated (\texttt{docs/paper2\_plan.md}, Task L1).} Mackie \textit{et al.} established
the experimental benchmark for $\beta$-casein at the air--water interface,
showing that its disordered chain forms an extended, viscoelastic layer
distinct from the more compact films formed by globular whey
proteins\citep{Mackie1999}, and the functional contrast between whey- and
casein-dominated films is well documented in foaming and emulsification
behaviour\citep{Dickinson1999,Foam4_2020}. This raises a direct molecular-scale
question that neither protein alone can answer: does a folded globular
protein with a buried hydrophobic feature (BLG's calyx) and an
intrinsically disordered protein with an exposed amphipathic terminus
($\beta$-casein's N-terminal region) represent two distinct adsorption
strategies, and can a structural characterisation of each be connected to
the established functional differences between whey and casein interfacial
films\citep{Gochev2019JPCB,Foam4_2020}?

Here we present a comparative atomistic MD study of native-state BLG and
$\beta$-casein at the air--water interface, designed to characterise --- not
to propose a unifying mechanism for --- the adsorption-relevant structural
features of each protein and to ground that characterisation in the
existing experimental phenomenology (Mackie, Cornec, Ulaganathan, Gochev).
This manuscript reports the BLG component, totalling \SI{4.00}{\micro\second} of
...
\begin{enumerate}
  ...
  \item the characterisation of the pre-commitment contact ensemble:
        ...
        rather than the global unfolding traditionally assumed; and
  \item \textbf{[DATA PENDING]} the establishment of this BLG
        characterisation as the empirical anchor for a direct comparison
        with $\beta$-casein, testing whether BLG's accessible-but-uncommitted
        calyx (the calyx-lining residues alone present
        $\SI{3.81 \pm 0.46}{\nano\metre\squared}$ of solvent-exposed
        surface throughout, \cref{sec:gate}) and $\beta$-casein's
        hypothesised open-chain adsorption are two distinct molecular
        strategies, each connectable to the established functional
        contrast between whey- and casein-dominated interfacial
        films\citep{Gochev2019JPCB,Foam4_2020,Mackie1999}.
\end{enumerate}
...
experimental adsorption kinetics\citep{Cornec1999,Ulaganathan2017a} from
...
\begin{figure}[tbp]
  \centering
  \includegraphics[width=0.88\textwidth,height=0.68\textheight,keepaspectratio]{../../results/figures/paper/PAPER_FIG2_CONTACT_AB.png}
...
\begin{figure}[tbp]
  \centering
  \includegraphics[width=0.88\textwidth,height=0.68\textheight,keepaspectratio]{../../results/figures/paper/PAPER_FIG2_CONTACT_CD.png}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.95\textwidth]{../../results/figures/paper/PAPER_FIG3_ACTIVATION.pdf}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.95\textwidth]{../../results/figures/paper/Fig4_optionA.pdf}
...
The SASA$_{12}$ measure above quantifies the exposure of the surrounding
hydrophobic patch; a complementary, more literal question is how exposed
the nine calyx-lining residues themselves are
(\numlist{39;41;56;58;92;103;105;107;125}; \SI{133}{} atoms). Computed the
same way (whole-protein freeSASA per PBC-corrected frame, summed over
calyx-residue atoms only; CENTER, \num{2001} frames at
\SI{0.5}{\nano\second} stride), this calyx-residue SASA is
$\SI{3.81 \pm 0.46}{\nano\metre\squared}$, ranging
\SIrange{2.8}{4.2}{\nano\metre\squared} with no excursion toward zero
(burial) or toward a markedly larger value (cooperative opening). The
calyx is therefore an \emph{accessible} structural feature of native BLG
on this timescale --- present and exposed throughout, rather than gated
open and closed --- which is the structural property that the
comparative framing with $\beta$-casein's open chain
(\cref{sec:casein}) is built on.
...
% =====================================================================
\section{$\beta$-Casein: an open-chain comparison}
\label{sec:casein}
% =====================================================================

\textbf{[DATA PENDING --- this section is a pre-registered analysis plan.
$\beta$-Casein system preparation (AlphaFold2 model, CHARMM36m, slab
geometry matched to the BLG systems above) begins in the next phase of
this study (\texttt{docs/paper2\_plan.md}); production MD and the
analyses below are not yet available. The subsections are structured to
mirror \cref{sec:contact-residency,sec:activation,sec:gate} so that the
BLG and $\beta$-casein results can be presented side by side in the final
manuscript.]}

\subsection{Contact and residency}
\label{sec:casein-contact}

\textbf{[DATA PENDING]} Contact and residency will be quantified exactly
as in \cref{sec:contact-residency}: single-heavy-atom minimum distance to
the nearest air--water interface, contact threshold \SI{0.30}{\nano\metre},
residency threshold \SI{10}{\nano\second}, PBC-corrected. Given
$\beta$-casein's lack of a folded core, the a priori expectation is a
higher contact fraction and/or shorter time-to-first-contact than BLG,
but --- consistent with the characterisation framing of this paper --- no
specific contact fraction is assumed in advance; the comparison with
BLG's \SIrange{7.1}{23.4}{\percent} per-trajectory contact fractions
(\cref{sec:contact-residency}) will be reported as observed.

\subsection{N-terminal region SASA: the ``open chain'' headline metric}
\label{sec:casein-nterm}

\textbf{[DATA PENDING]} The headline structural metric for $\beta$-casein,
designed as the direct counterpart to the calyx-residue SASA reported in
\cref{sec:gate} ($\SI{3.81 \pm 0.46}{\nano\metre\squared}$, BLG calyx-lining
residues), is the solvent-accessible surface area of the strongly
amphipathic N-terminal region (residues 1--25), computed the same way:
whole-chain freeSASA per PBC-corrected frame, summed over the N-terminal
residues' atoms, at \SI{0.5}{\nano\second} stride. The target presentation
is a single comparative row: ``Accessible-feature SASA: BLG calyx
$\SI{3.81 \pm 0.46}{\nano\metre\squared}$ vs.\ $\beta$-casein N-term~1--25
$X \pm Y\,\si{\nano\metre\squared}$.'' If $X$ is large and stable like the
BLG calyx value, both proteins present a persistently accessible
hydrophobic/amphipathic feature despite radically different folds
(``two routes to the same accessibility''); if $X$ is small or highly
variable, the two proteins' adsorption-relevant surfaces behave
qualitatively differently. Either outcome is reportable under the
characterisation framing of this study --- this metric is not assumed in
advance to validate the ``open chain'' framing of the title, which is
itself a hypothesis to be tested by this measurement.

\subsection{Whole-chain SASA vs.\ an extended-chain reference}
\label{sec:casein-wholechain}

\textbf{[DATA PENDING]} As a secondary, contextual measure, total
$\beta$-casein SASA near the interface will be compared against an
extended-chain reference conformation (e.g., the AlphaFold2 starting
model, or a fully extended idealised chain of the same sequence). This
addresses whether $\beta$-casein collapses toward a more globular SASA
near the interface or remains in an extended, ``open'' configuration ---
providing context for the N-terminal headline metric
(\cref{sec:casein-nterm}) rather than standing alone.

\subsection{Secondary structure (DSSP)}
\label{sec:casein-dssp}

\textbf{[DATA PENDING]} DSSP coil-fraction analysis, using the same
MDAnalysis~2.10 DSSP implementation applied to BLG, will be reported as a
sanity check: a high coil fraction is the expected signature of an IDP
and is not, on its own, a novel finding. It is included for completeness
and to detect any unexpected transient secondary-structure formation at
the interface, which --- if observed --- would be a genuinely interesting
result worth elevating beyond a sanity check.
...
\subsection*{Toward a comparative framework: accessible calyx vs.\ open chain}

The framing established above --- characterisation of a structural
ensemble and a precise statement of the simulation boundary, rather than
a proposed causal mechanism for commitment --- is deliberate, and it is
the framing this comparative study is built on. An earlier two-factor
``gate'' hypothesis (calyx SASA and orientation jointly gating
commitment) was tested directly and is not supported by the data
($r = +0.006$, \cref{sec:gate}); we do not resurrect a mechanistic claim
here. What the data do support, robustly, is a structural fact: the BLG
calyx --- both the surrounding hydrophobic patch (SASA$_{12}$,
\SIrange{24}{37}{\nano\metre\squared}) and the nine calyx-lining residues
themselves (SASA$_\mathrm{calyx}$, $\SI{3.81 \pm 0.46}{\nano\metre\squared}$)
--- is persistently \emph{accessible} throughout \SI{4.00}{\micro\second},
without committing to adsorption.

This accessible-but-uncommitted calyx is one half of the comparison this
paper sets out to make (\cref{sec:casein}). The other half --- whether
$\beta$-casein's disordered chain presents its amphipathic N-terminal
region with comparable persistence (\cref{sec:casein-nterm}) --- is
\textbf{[DATA PENDING]}. The application-relevant link, raised explicitly
by our supervisor (P.P.), is to connect this molecular-scale
characterisation to measurable interfacial behaviour: Gochev
\textit{et al.}\citep{Gochev2019JPCB,Foam4_2020} report BLG adsorbed-layer
structure (neutron reflectometry) and foam stability (tensiometry) for
the same protein characterised here, and Mackie \textit{et al.}\citep{Mackie1999}
report the corresponding interfacial rheology for $\beta$-casein. The
intended comparative argument is therefore not ``our MD predicts a new
mechanism for foam stability'' but ``the structural contrast we
characterise at the molecular scale --- a folded protein with a
persistently accessible hydrophobic feature versus a disordered chain
with a persistently exposed amphipathic terminus --- maps onto the
already-documented functional contrast between whey- and casein-dominated
interfacial films.'' Establishing whether that mapping holds is the
purpose of \cref{sec:casein}, and the comparative discussion and
conclusion of this manuscript are written once those results are
available.
...
timescale\citep{Cornec1999,Ulaganathan2017a}. The contact/commitment
...
tension by roughly half relative to TIP4P/2005 and to
...
sampling and lies outside the scope of this study. Sixth, the $\beta$-casein comparison (\cref{sec:casein}) that motivates
the title of this manuscript is, as of this draft, an analysis plan
rather than a result --- the central limitation of this version of the
paper, to be resolved before submission. Fat-soluble components and the
$\beta$-casein--Ca$^{2+}$ bridge, both relevant in real milk systems, are
addressed in subsequent papers of this programme.
...
\subsection*{$\beta$-Casein system (planned)}

\textbf{[DATA PENDING --- parameters below are proposals, to be confirmed
before any production run; see \texttt{docs/paper2\_plan.md}.]} The
starting structure will be an AlphaFold2 model of bovine $\beta$-casein
(low-pLDDT, as expected for an IDP --- a single representative
conformation rather than a disorder ensemble, a known limitation noted
in the Discussion). System preparation will follow the same protocol as
the BLG systems above: CHARMM36m force field (chosen in part for its
established performance on intrinsically disordered
proteins\citep{Huang2017}), TIP3P water, slab geometry matched to the
BLG box ($12\times12\times35\,\si{\nano\metre}$, two air--water
interfaces), Na$^+$/Cl$^-$ at physiological ionic strength
(\SI{150}{\milli\Molar}), and the same five-stage equilibration (energy
minimisation; restrained NVT; NPT; unrestrained NVT slab; production,
no pressure coupling). The number of replicas and total simulation time
are TBD pending discussion with the supervisor; \texttt{docs/paper2\_plan.md}
proposes \num{2}--\num{3} replicas of $\geq$\SI{1000}{\nano\second} each,
matched in order of magnitude to the \SI{4.00}{\micro\second} BLG
aggregate above.
...
definition. (iii)~\textit{SASA$_\mathrm{calyx}$}
(\cref{sec:gate}, final paragraph): SASA computed for the whole protein
per PBC-corrected frame (as above), then summed over only the nine
calyx-lining residue atoms
(\numlist{39;41;56;58;92;103;105;107;125}, \num{133} atoms) --- a
literal measure of calyx exposure, distinct from and smaller than
SASA$_8$/SASA$_{12}$, which include the surrounding hydrophobic patch.
The planned $\beta$-casein N-terminal-region SASA
(\cref{sec:casein-nterm}) will use the same whole-chain-then-subset
procedure for direct comparability.
...
\url{https://github.com/cpornjar/blg-airwater-interface-md}
